...
| =symbol_exists=          | Emacs Runtime   | yes          | no           | no           |
| =function_documentation= | Emacs Runtime   | yes          | no           | no           |
| =variable_documentation= | Emacs Runtime   | yes          | no           | no           |
...
| =eval_elisp=             | Emacs Runtime   | unrestricted | unrestricted | always       |
...
| =list_files=             | File Management | yes          | no           | no           |
| =list_project_files=     | File Management | yes          | no           | no           |
| =search_files=           | Emacs Runtime   | yes          | no           | no           |
| =search_project_files=   | File Management | yes          | no           | no           |
| =read_file=              | File Management | yes          | no           | no           |
| =read_project_file=      | File Management | yes          | no           | no           |
| =edit_file=              | Emacs Runtime   | yes          | yes          | always       |
| =rename_file=            | Emacs Runtime   | yes          | yes          | always       |
| =delete_file=            | Emacs Runtime   | yes          | yes          | always       |
| =create_directory=       | Emacs Runtime   | yes          | yes          | always       |
| =run_shell_command=      | Shell           | unrestricted | unrestricted | always       |
| =man_page=               | Shell           | yes          | no           | no           |
| =search_web=             | Web Tools       | yes          | no           | no           |
| =read_webpage=           | Web Tools       | yes          | no           | no           |
...
(from https://github.com/karthink/gptel-agent/blob/master/gptel-agent-tools.el)
...
#+begin_note
This tool has no command whitelist/blacklist and can run anything the shell
allows. :confirm t means you see the command before it runs, but the
post-tool-call hook below only logs the tool *name*, not the command itself — so
there's no audit trail afterwards. Consider either restricting this (allow-list
of prefixes, or a dedicated safe subset of commands) or extending the logging
hook to record the actual command for this tool specifically.
#+end_note
